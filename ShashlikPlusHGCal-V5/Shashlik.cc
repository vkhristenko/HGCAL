...
//	CLHEP::HepRandom::setTheSeed(time(NULL));
	...
	sprintf(rootOutFile, "HGCal_%d_%d_%d_%d.root", runParams.iEnergy,
	...
	shFile->cd();
	shFile->mkdir("SHASHLIK");
	shFile->cd("SHASHLIK");
	TTree *shashlikTree = new TTree("detData", "detData");
	shFile->cd();
	shFile->mkdir("PRIM");
	shFile->cd("PRIM");
...
//	SHPhysicsList *phys = new SHPhysicsList();
	...
	SHPrimaryGeneratorAction *primAction = 
		new SHPrimaryGeneratorAction(runParams, primTree);
